# @no-bash-mcp/linux-arm64 (Platform package)

The **Platform package** carrying the `linux-arm64` (aarch64) **native binary** of the no-bash-mcp MCP
server. npm installs it only on a matching host (`os: ["linux"]`, `cpu: ["arm64"]`); the
